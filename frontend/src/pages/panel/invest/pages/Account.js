...
import { VerifyAccount } from "../../../../appRedux/actions/CustomerAction";
import { VERIFY_ACCOUNT_RESET } from "../../../../appRedux/actions/type";
import { filterDay,filterMonth,yearsArray } from "./dropData";
...
const CustomerAccount = () => {
  ...
  const VerifyAccounttate = useSelector((state) => state.verifyCustomerAccount);

  const { verifyloading, verifyerror, verify } = VerifyAccounttate;
  ...
  const onFormSubmit = (submitData) => {
    ...
    dispatch(VerifyAccount(submittedData));
    // resetForm()
  ...
  useEffect(() => {
    ...
    if (verifyerror) {
      Swal.fire({
        ...
        if (value.isDismissed || value.isConfirmed) {
          reset();
    ...
    if (verify?.Status) {
     history.push({pathname:'/kyc/otp'})
     dispatch({ type: VERIFY_ACCOUNT_RESET });
    }else{
      
    }
  }, [history,dispatch, verifyerror, verify]);
    
  const { errors, register, handleSubmit,reset } = useForm();
  return (
    <React.Fragment>
      <Head title="CustomerAccount"></Head>
      <Content size="lg">
        <BlockHead size="lg" className="wide-xs mx-auto">
          <BlockHeadContent className="text-center">
            <BlockTitle className="fw-normal" tag="h2">
              {/* Nice, Abu Bin Ishtiyak ! */}
              <img src={Logo} class="logo-dark logo-img" />
            </BlockTitle>
            {/* <BlockDes>
              <p>
                 ...
                 <div className="card-inner">
                  ...
                  <div className="card-inner">
                    <ul className="list list-step">
                      ...
                      <li>Enter your CalBank Account number and Date of Birth.</li>
                      <li>You will receive a One Time Password (OTP), via the phone number/ email registered on your account.</li>
                      <li>Enable camera on your device to take Selfie (take a live passport image of yourself).</li>
                      <li>Enter Ghana Card Number. E.g GHA-1234567890-1.</li>
                      <li>Upload a copy of the front and back images of your Ghana Card. (jpg/png format).</li>
                      <li>You will receive notification once the request is approved.</li>
                    </ul>
                  </div>
                  <div className="card-inner">
                    {/* <div className="align-center gx-3">
                      ...
                      <div className="flex-item">
                        <span className="sub-text sub-text-sm text-soft">1/3 Completed</span>
                      </div>
                    </div> */}
                  </div>
                </div>
              ...
              <Col lg="6">
                <div className="card-inner card-inner-lg h-100">
                  ...
                  <div className="block-image w-200px flex-shrink-0 order-first order-md-last">
                    ...
                    <BlockContent>
                      <BlockHeadContent>
                        <h4>Verify Your Account Number </h4>
              ...
              <Row className="g-4">
                ...
                <BlockContent className="flex-shrink-0">
                   ...
                   {/* {verifyloading ? <Spinner size="sm" color="yellow" m /> : <Button color="white" size="lg" className="btn-block  btn-outline-warning btn-dim"   outline type="submit">Verify
                     ...
                     <Button size="lg" className="btn-block  btn-warning btn-dim" type="submit" disabled={verifyloading} color="warning">
